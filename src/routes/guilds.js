...
=======
        if (req.body.name && req.body.name.length < 31) {
            const guild = {
                id: flake.gen().toString(),
                name: req.body.name,
                channels: [{ id: flake.gen().toString(), name: 'general', topic: null, position: 0, messages: [], pins: [] }],
                roles: [{ id: 0, name: 'Owner', permissions: 4095, color: '#FFC0CB', hoist: false, position: 0 }],
...
=======
    ...
    app.patch('/guilds/*', async (req, res) => {
        const guildId = Object.values(req.params)
            .map((x) => x.replace(/\//g, ''))
            .filter((x) => {
                return x != '';
    ...
    app.delete('/guilds/*', async (req, res) => {
        ...
        if (guildId) {
            database.query(`SELECT * FROM guilds`, async (err, dbRes) => {
                ...
                database.query(`DELETE FROM guilds WHERE id = '${guildId}'`, async (err, dbRes) => {
                            if (!err) {
                                guild.members.forEach(member => {
                                    websockets.get(member)?.forEach(websocket => {
